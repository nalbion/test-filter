 ...
 * @param {{user: string, repo: string, headers: Object=, milestone: string=, state: string=,
  ...
  * since: Date=, page: number=, per_page: number=}} options - accepts an object with the following fields:
...
GitHubApi.prototype.getIssues = function (options) {
    ...
    this.github.issues.repoIssues(options, function (error, data) {
        if (error) {
            ...
            console.info('Downloaded', _.keys(issues).length, 'issues from GitHub: ', options.group, options.repo);
...
GitHubApi.prototype.parseIssue = function (issues, record) {
    issues[record.number] = new GitHubIssue(record);
    return issues;
};
